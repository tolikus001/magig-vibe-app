...
export function renderForecastResultScreen(container, state, handlers) {
  ...
  container.innerHTML = `
    <div class="max-w-md mx-auto min-h-screen flex flex-col justify-between p-5 slide-up">
      <div class="pt-2">
        <div class="flex items-center justify-between mb-3 bg-slate-900/90 border border-slate-800 p-3 rounded-xl">
          <div>
        ...
        <div class="bg-amber-950/30 border border-amber-500/30 rounded-xl p-3 mb-4 text-[11px] text-slate-300 space-y-1">
          ...
          <p>• <strong>Гексаграмма:</strong> Архетип вашей энергии сегодня (баланс ума и обстоятельств).</p>
          <p>• <strong>Инсайт МАГиГ:</strong> Психотерапевтическая установка — куда направить внимание ума.</p>
          <p>• <strong>Применение:</strong> Используйте блок «Что сделать» для задач и повторите аффирмацию.</p>
        </div>

        <div class="card bg-gradient-to-br from-amber-950/40 via-slate-900/95 to-slate-900/95 border-amber-500/40 p-4.5 mb-4 shadow-xl shadow-amber-500/10">
          <div class="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              <h3 class="text-base font-bold text-slate-100 leading-snug">${h.nameRu}</h3>
          ...
          <div class="grid grid-cols-2 gap-2 text-xs bg-slate-950/70 p-2.5 rounded-xl border border-slate-800/80 mb-3">
            <div><span class="text-[9px] text-slate-400 font-semibold uppercase">Верхняя Триграмма</span><div class="font-bold text-amber-300 text-[11px]">${h.upperTrigram.symbol} ${h.upperTrigram.name}</div></div>
            <div><span class="text-[9px] text-slate-400 font-semibold uppercase">Нижняя Триграмма</span><div class="font-bold text-amber-300 text-[11px]">${h.lowerTrigram.symbol} ${h.lowerTrigram.name}</div></div>
          </div>

          <div class="bg-amber-500/10 border-l-2 border-amber-400 p-2.5 rounded-r-xl text-xs text-amber-200/90 italic mb-2.5">${h.quote}</div>
        ...
        <div class="card bg-gradient-to-br from-indigo-950/50 via-slate-900/90 to-purple-950/50 border-indigo-500/40 p-4 mb-4 shadow-md">
          <div class="flex items-center gap-2 mb-2">
            ...
            <div><h4 class="text-xs font-bold text-indigo-300 uppercase">Инсайт Бессознательного (МАГиГ)</h4><span class="text-[10px] text-slate-400">Практический фокус для ума</span></div>
          </div>
        ...
        <div class="card bg-slate-900/80 border-slate-800 p-3.5 mb-5 text-xs space-y-2">
          <div class="flex justify-between items-center text-amber-300 font-bold border-b border-slate-800 pb-1.5"><span>✨ Талисман Дня</span><span class="text-[10px] text-slate-400">Стихия: ${h.talisman.element}</span></div>
          ...
          <div class="bg-amber-500/10 p-2 rounded-lg border border-amber-500/20 text-center font-medium text-amber-200 text-[11px]">«${h.talisman.affirmation}»</div>
        </div>
      </div>
